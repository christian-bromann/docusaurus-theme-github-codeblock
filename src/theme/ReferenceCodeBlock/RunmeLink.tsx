...
import { parseReference } from './utils.js'
...
export interface RunmeLinkProps extends PropsWithChildren {
    reference: string
    metastring?: string
}

export function RunmeLink (props: RunmeLinkProps) {
    return <BrowserOnly>
        {() => <RunmeLinkComponent {...props} />}
    </BrowserOnly>
}

function RunmeLinkComponent({ reference, metastring }: RunmeLinkProps) {
    const userAgent = (
        navigator.userAgent ||
        navigator.vendor ||
        // @ts-expect-error
    ...
    return (
        <a
            href={`vscode://stateful.runme?${getRunmeLink(reference, metastring)}`}
            className='runmeLink'
            target="_blank"
            style={buttonStyles}
        >
